    ...
    case $dozzle_choice in
      ...
      2)
        ...
        if sudo docker ps --format '{{.Names}}' | grep -q '^dozzle$'; then
          ...
          echo "💡 Use option 1 to install it."
        fi
        read -p "Press Enter to continue..."
        ;;

      3)
        echo -e "${RED}⚠️ This will delete the Dozzle container completely.${NC}"
        ...
        if [[ "$confirm" =~ ^[Yy]$ ]]; then
          if sudo docker ps -a --format '{{.Names}}' | grep -q '^dozzle$'; then
            ...
            sudo ufw delete allow 9999 >/dev/null 2>&1
            sudo ufw reload >/dev/null 2>&1
            echo "✅ Dozzle container deleted."
          else
            echo "⚠️ Dozzle container not found."
          fi
        else
          echo "❌ Cancelled."
        fi
